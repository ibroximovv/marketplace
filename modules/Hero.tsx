...
interface HeroType {
  auctions: { data: AuctionType[], meta: MetaType },
  artists: { data: ArtistType[], meta: MetaType },
  actionResults: { data: any[], meta: MetaType }
}

const Hero: FC<HeroType> = ({ auctions, artists, actionResults }) => {
  const t = useTranslations("HeroContent")
  const { data: auctionsList, isLoading: actionLoading } = getQueryData("/auctions", auctions, 'auctions')
      ...
      <div className='containers  flex flex-col md:flex-row gap-[10px] justify-between'>
        <div className='md:w-[50%]'>
          ...
          <div className='!mt-[20px] lg:mt-[36px] flex gap-[30px]'>
            ...
            <div>
              <Heading tag='h3'>{actionLoading ? "0" : auctionsList.meta.total}</Heading>
        ...
        <div className='mx-auto md:mx-0 mt-[40px] md:mt-0 md:w-[330px] lg:w-[510px] self-start bg-[#3B3B3B] rounded-[20px] overflow-hidden'>
          <Image className='w-[510px] h-[221px] object-cover lg:h-[401px]' src={'/hero-img.svg'} alt='Hero img' width={510} height={401} priority />
          <div className='py-[22px] px-[20px]'>
            <Heading classList='!text-[22px]' tag='h3'>Space Walking</Heading>
            <div className='flex items-center gap-[12px] mt-[10px]'>
              <Image className='w-[24px] h-[24px]' src={'/animaked.svg'} alt='Animakid img' width={24} height={24} priority />
